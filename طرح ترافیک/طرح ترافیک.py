...
def expiry_checker(date1,date2):
    _date1=[]
    _date2=[]
    for d in date1.split("/"):
        _date1.append(int(d))
    _year1,_month1,_day1 = _date1[0],_date1[1],_date1[2]
    for d1 in date2.split("/"):
        _date2.append(int(d1))
    _year2,_month2,_day2 = _date2[0],_date2[1],_date2[2]
    
    if _year1-_year2 > 0:
        return True
    elif _month1 - _month2 > 0:
        return True
    elif _day1 - _day2 > 0:
        return True
    else:
...
while is_running:
    ...
    if command == "REGISTER":   
        ...
        if data[0] not in car_number_list:
            ...
            if expiry_dir != {}:
                has_license = expiry_checker( expiry_dir.get(data[0]),data[1])
            else:
                has_license = False
